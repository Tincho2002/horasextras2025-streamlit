        ...
        with st.spinner("Generando desgloses organizacionales..."):
            ...
            with st.container(border=True):
                ...
                if df_grouped_gm.empty: st.warning("No hay datos para 'Gerencia y Ministerio' con los filtros seleccionados.")
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        y_axis = alt.Y('Gerencia:N', sort=alt.SortField(field="Total_Costos", op="sum", order="descending"), title="Gerencia")
                        bars = alt.Chart(df_grouped_gm).mark_bar().encode(x=alt.X('Total_Costos:Q', title="Total Costos ($)"), y=y_axis, color='Ministerio')
                    ...
                    with col2:
                        y_axis = alt.Y('Gerencia:N', sort=alt.SortField(field="Total_Cantidades", op="sum", order="descending"), title="Gerencia")
                        bars = alt.Chart(df_grouped_gm).mark_bar().encode(x=alt.X('Total_Cantidades:Q', title="Total Cantidades"), y=y_axis, color='Ministerio')
            ...
            with st.container(border=True):
                ...
                if not df_grouped_gs.empty:
                    ...
                    with col1:
                        y_axis = alt.Y('Gerencia:N', sort=alt.SortField(field="Total_Costos", op="sum", order="descending"), title="Gerencia")
                        bars = alt.Chart(df_grouped_gs).mark_bar().encode(x=alt.X('Total_Costos:Q', title="Total Costos ($)"), y=y_axis, color='Sexo')
                    ...
                    with col2:
                        y_axis = alt.Y('Gerencia:N', sort=alt.SortField(field="Total_Cantidades", op="sum", order="descending"), title="Gerencia")
                        bars = alt.Chart(df_grouped_gs).mark_bar().encode(x=alt.X('Total_Cantidades:Q', title="Total Cantidades"), y=y_axis, color='Sexo')
            ...
            with st.container(border=True):
                ...
                if not df_grouped_ms.empty:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        y_axis = alt.Y('Ministerio:N', sort=alt.SortField(field="Total_Costos", op="sum", order="descending"), title="Ministerio")
                        bars = alt.Chart(df_grouped_ms).mark_bar().encode(x=alt.X('Total_Costos:Q', title="Total Costos ($)"), y=y_axis, color='Sexo')
                        ...
                        st.altair_chart((bars + text).properties(title='Costos').interactive(), use_container_width=True)
                    with col2:
                        y_axis = alt.Y('Ministerio:N', sort=alt.SortField(field="Total_Cantidades", op="sum", order="descending"), title="Ministerio")
                        bars = alt.Chart(df_grouped_ms).mark_bar().encode(x=alt.X('Total_Cantidades:Q', title="Total Cantidades"), y=y_axis, color='Sexo')
            ...
            with st.container(border=True):
                ...
                if not df_grouped_ns.empty:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        y_axis = alt.Y('Nivel:N', sort=alt.SortField(field="Total_Costos", op="sum", order="descending"), title="Nivel")
                        bars = alt.Chart(df_grouped_ns).mark_bar().encode(x=alt.X('Total_Costos:Q', title="Total Costos ($)"), y=y_axis, color='Sexo')
                        ...
                        st.altair_chart((bars + text).properties(title='Costos').interactive(), use_container_width=True)
                    with col2:
                        y_axis = alt.Y('Nivel:N', sort=alt.SortField(field="Total_Cantidades", op="sum", order="descending"), title="Nivel")
                        bars = alt.Chart(df_grouped_ns).mark_bar().encode(x=alt.X('Total_Cantidades:Q', title="Total Cantidades"), y=y_axis, color='Sexo')
            ...
            with st.container(border=True):
                ...
                if not df_grouped_fs.empty:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        y_axis = alt.Y('Función:N', sort=alt.SortField(field="Total_Costos", op="sum", order="descending"), title="Función")
                        bars = alt.Chart(df_grouped_fs).mark_bar().encode(x=alt.X('Total_Costos:Q', title="Total Costos ($)"), y=y_axis, color='Sexo')
                        ...
                        st.altair_chart((bars + text).properties(title='Costos').interactive(), use_container_width=True)
                    with col2:
                        y_axis = alt.Y('Función:N', sort=alt.SortField(field="Total_Cantidades", op="sum", order="descending"), title="Función")
                        bars = alt.Chart(df_grouped_fs).mark_bar().encode(x=alt.X('Total_Cantidades:Q', title="Total Cantidades"), y=y_axis, color='Sexo')
