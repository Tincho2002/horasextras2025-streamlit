...
with tab2:
    ...
    dimension_options = {
        'Gerencia y Ministerio': ['Gerencia', 'Ministerio'],
        'Gerencia y Sexo': ['Gerencia', 'Sexo'],
        'Ministerio y Sexo': ['Ministerio', 'Sexo'],
        'Nivel y Sexo': ['Nivel', 'Sexo'],
        'Función y Sexo': ['Función', 'Sexo']
    }
    selected_dimension_key = st.selectbox(
        'Selecciona una vista de desglose:',
    ...
    with st.spinner(f"Generando desglose por {selected_dimension_key}..."):
        df_grouped = calculate_grouped_aggregation(
            df, 
            st.session_state.final_selections, 
            group_cols, 
            cost_columns_options, 
            quantity_columns_options, 
            st.session_state.cost_types_ms, 
            st.session_state.quantity_types_ms
        )
        ...
        if df_grouped.empty:
            ...
            with st.container(border=True):
                ...
                total_row[primary_col] = 'TOTAL'
                total_row[secondary_col] = ''
                ...
                with col1:
                    ...
                    bars = alt.Chart(df_grouped).mark_bar().encode(
                        ...
                        y=y_axis,
                        color=f'{secondary_col}:N',
                        tooltip=[
                            primary_col,
                            secondary_col,
                            alt.Tooltip('sum(Total_Costos):Q', format='$,.2f', title='Costo')
                        ]
                    )
                    st.altair_chart(bars.properties(title='Costos').interactive(), use_container_width=True)
                ...
                with col2:
                    ...
                    bars = alt.Chart(df_grouped).mark_bar().encode(
                        ...
                        tooltip=[
                            primary_col,
                            secondary_col,
                            alt.Tooltip('sum(Total_Cantidades):Q', format=',.0f', title='Cantidad')
                        ]
                    )
                    st.altair_chart(bars.properties(title='Cantidades').interactive(), use_container_width=True)
